...
### Fixed
- Fixed startup crash in v1.6.1 caused by stale manual `AssemblyInfo.cs` version metadata.
- Removed the broken single-file WPF standalone asset from release packaging; installer and portable zip are smoke-tested.

## [1.6.1] - 2026-07-05

### Added
- Run one selected macro across all Chrome windows from Dashboard.
- Public demo badge and commercial contact/custom automation page.

### Changed
- Polished Dashboard action bar and shared WPF card/button styling for a more professional public build.
- Reused parallel executor event wiring to avoid duplicate status/log handlers.
